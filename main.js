...
function gotResults(erro, results) {
    ...
    console.log(results)
    var resultTexto = results[0].label
    document.getElementById("label").innerHTML = "nome: " + resultTexto.replace("_", " ")
    document.getElementById("confidence").innerHTML = "precisão: " + Math.round(results[0].confidence * 100) + "%"

    utterThis = new SpeechSynthesisUtterance(resultTexto.replace("_", " "))
    synth.lang = "pt-BR"
    synth.speak(utterThis)
    fala = ""

    if (resultTexto == "skull") {
